...
image_color = cv2.imread("C:/Users/ascarneiro/Desktop/TCC/ScoreReader/repository/staffless.png")
#image_color = cv2.bilateralFilter(image_color,9,75,75)
...
for c in contours:
    ...
    if r >= 15 and r <= 25:
        ...
        cv2.putText(image_color, "(" + str(int(realX)) + " , " + str(int(realY)) + ")", (int(cX), int(cY + add)),
                       cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 0, 0), 1, cv2.LINE_AA)

        figuras.append(
            {"ponto": {"index": index, "x": (int(realX)), "y": (int(realY)), "center": center, "raio": r}})

    index = index + 1

cv2.imwrite('C:/Users/ascarneiro/Desktop/TCC/ScoreReader/repository/1.png',image)
